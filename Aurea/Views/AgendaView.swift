...
struct AgendaView: View {
    ...
    private var filteredItems: [AgendaItem] {
        items.filter { item in
            ...
            let searchOK = searchText.isEmpty || item.title.localizedCaseInsensitiveContains(searchText) || item.note.localizedCaseInsensitiveContains(searchText)
    ...
    var body: some View {
        NavigationStack {
            List {
                Section {
                    HStack {
                        ...
                        Spacer(); Button("Oggi") { selectedDate = .now }.fontWeight(.semibold); Spacer()
                ...
                Section {
                    Picker("Filtro", selection: $filter) { ForEach(AgendaFilter.allCases) { Label($0.title, systemImage: $0.icon).tag($0) } }.pickerStyle(.menu)
                }

                Section {
                    HStack(spacing: 16) {
                        dayMetric(title: "Impegni", value: "\(activeDayItems.count)"); Divider().frame(height: 34)
                        dayMetric(title: "Entrate", value: dayIncome.formatted(.currency(code: "EUR"))); Divider().frame(height: 34)
                ...
                Section {
                    if activeDayItems.isEmpty { ContentUnavailableView("Nessun impegno", systemImage: "calendar.badge.checkmark", description: Text(searchText.isEmpty ? "Non ci sono attività aperte per questa giornata." : "Nessun risultato per la ricerca.")) }
            ...
            .searchable(text: $searchText, prompt: "Cerca in agenda")
    ...
    private func moveDay(_ amount: Int) { selectedDate = Calendar.current.date(byAdding: .day, value: amount, to: selectedDate) ?? selectedDate }
    private func agendaSort(_ lhs: AgendaItem, _ rhs: AgendaItem) -> Bool {
        let rank: [AgendaPriority:Int] = [.high:0,.normal:1,.low:2]
        if lhs.priority != rhs.priority { return (rank[lhs.priority] ?? 1) < (rank[rhs.priority] ?? 1) }
...
struct AgendaItemDetailView: View {
    ...
    }.navigationTitle("Dettaglio").navigationBarTitleDisplayMode(.inline).toolbar { ToolbarItem(placement: .cancellationAction) { Button("Chiudi") { dismiss() } }.self }.sheet(isPresented: $showingEdit) { EditAgendaItemView(item: item) } } }
