...
struct AgendaView: View {
    ...
    private var filteredItems: [AgendaItem] {
        items.filter { item in
            let typeOK = filter.matches(item)
            let searchOK = searchText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || item.title.localizedCaseInsensitiveContains(searchText) || item.note.localizedCaseInsensitiveContains(searchText)
    ...
    private var overdueItems: [AgendaItem] {
        ...
        return filteredItems.filter { $0.date < todayStart && !$0.isCompleted && ($0.type == .task || $0.type == .deadline) }.sorted(by: agendaSort)
    }
                ...
                if isSearching {
                    Section("Risultati") {
                        if searchResults.isEmpty {
                            ContentUnavailableView("Nessun risultato", systemImage: "magnifyingglass", description: Text("Nessun impegno corrisponde alla ricerca."))
                        } else {
                            ForEach(searchResults) { item in
                                Button { selectedDate = item.date; displayedMonth = item.date; selectedItem = item } label: {
                                    HStack {
                                        agendaRowContent(item)
                                        VStack(alignment: .trailing, spacing: 2) {
                                            Text(item.date.formatted(date: .abbreviated, time: .omitted)).font(.caption)
                                            if item.hasTime { Text(item.date.formatted(date: .omitted, time: .shortened)).font(.caption2) }
                    ...
                    Section {
                        HStack {
                            ...
                            Spacer(); Button("Oggi") { goToToday() }.fontWeight(.semibold); Spacer()
    ...
    private func moveDay(_ amount: Int) { if let newDate = Calendar.current.date(byAdding: .day, value: amount, to: selectedDate) { selectedDate = newDate; displayedMonth = newDate } }
    private func goToToday() { selectedDate = .now; displayedMonth = .now }
    private func agendaSort(_ lhs: AgendaItem, _ rhs: AgendaItem) -> Bool { let rank: [AgendaPriority:Int] = [.high:0,.normal:1,.low:2]; if lhs.priority != rhs.priority { return (rank[lhs.priority] ?? 1) < (rank[rhs.priority] ?? 1) }; return lhs.date < rhs.date }
    ...
    private func agendaRowContent(_ item: AgendaItem) -> some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                HStack(spacing: 5) { Text(item.type.title); if item.hasTime { Text("•"); Text(item.date.formatted(date: .omitted, time: .shortened)) }; if item.repeatRule != .never { Image(systemName: "repeat") }; if item.reminderMinutesBefore != nil && item.hasTime { Image(systemName: "bell") }; if item.priority != .normal { Text("• \(item.priority.title)") } }.font(.caption).foregroundStyle(.secondary)
            }.foregroundStyle(item.isCompleted ? .secondary : .primary)
        }
    }
    private func normalizeRecurringEvents() { for item in items where item.type == .event && item.repeatRule != .never && item.date < Date() { var next = item.date; while next < Date() { next = item.repeatRule.nextDate(after: next) }; item.date = next; AgendaNotificationManager.schedule(for: item) } }
}
...
struct AddAgendaItemView: View {
    @Environment(\.dismiss) private var dismiss; @Environment(\.modelContext) private var modelContext; let defaultDate: Date
    @State private var title = ""; @State private var note = ""; @State private var type: AgendaItemType = .task; @State private var date: Date; @State private var hasTime = false; @State private var repeatRule: AgendaRepeat = .never; @State private var reminder = 0; @State private var priority: AgendaPriority = .normal
    init(defaultDate: Date = .now) { self.defaultDate = defaultDate; _date = State(initialValue: defaultDate) }
    var body: some View { NavigationStack { Form { Section("Tipo") { Picker("Tipo", selection: $type) { ForEach(AgendaItemType.allCases) { Label($0.title, systemImage: $0.icon).tag($0) } }.pickerStyle(.segmented) }; Section("Dettagli") { TextField("Titolo", text: $title); TextField("Note (opzionale)", text: $note, axis: .vertical); DatePicker("Data", selection: $date, displayedComponents: .date); Toggle("Orario", isOn: $hasTime); if hasTime { DatePicker("Ora", selection: $date, displayedComponents: .hourAndMinute) }; Picker("Priorità", selection: $priority) { ForEach(AgendaPriority.allCases) { Text($0.title).tag($0) } } }; Section("Ripetizione") { Picker("Ripeti", selection: $repeatRule) { ForEach(AgendaRepeat.allCases) { Text($0.title).tag($0) } } }; Section { Picker("Avviso", selection: $reminder) { Text("Nessuno").tag(0); Text("All'ora dell'impegno").tag(1); Text("5 minuti prima").tag(5); Text("15 minuti prima").tag(15); Text("30 minuti prima").tag(30); Text("1 ora prima").tag(60); Text("1 giorno prima").tag(1440) }.disabled(!hasTime) } header: { Text("Promemoria") } footer: { Text(hasTime ? "Aurea programmerà una notifica locale sul dispositivo." : "Attiva Orario per impostare un promemoria.") } }.navigationTitle("Nuovo impegno").navigationBarTitleDisplayMode(.inline).toolbar { ToolbarItem(placement: .cancellationAction) { Button("Annulla") { dismiss() } }; ToolbarItem(placement: .confirmationAction) { Button("Salva") { save() }.disabled(title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty) } } } }
    private func save() { let item = AgendaItem(title: title.trimmingCharacters(in: .whitespacesAndNewlines), note: note.trimmingCharacters(in: .whitespacesAndNewlines), type: type, date: date, hasTime: hasTime, repeatRule: repeatRule, reminderMinutesBefore: hasTime && reminder != 0 ? reminder : nil, priority: priority); modelContext.insert(item); AgendaNotificationManager.schedule(for: item); dismiss() }
}
...
struct AgendaItemDetailView: View {
    ...
    var body: some View { NavigationStack { Form { Section("Impegno") { LabeledContent("Tipo", value: item.type.title); LabeledContent("Titolo", value: item.title); LabeledContent("Data", value: item.hasTime ? item.date.formatted(date: .long, time: .shortened) : item.date.formatted(date: .long, time: .omitted)); LabeledContent("Priorità", value: item.priority.title); if !item.note.isEmpty { LabeledContent("Note", value: item.note) }; if item.repeatRule != .never { LabeledContent("Ripetizione", value: item.repeatRule.title) } }; Section { Button { showingEdit = true } label: { Label("Modifica impegno", systemImage: "pencil") } }; if item.type == .task || item.type == .deadline { Section { Button { toggleCompletion() } label: { Label(completionTitle, systemImage: item.isCompleted ? "arrow.uturn.backward.circle" : "checkmark.circle") } } }; Section { Button(role: .destructive) { AgendaNotificationManager.remove(for: item); modelContext.delete(item); dismiss() } label: { Label("Elimina", systemImage: "trash") } } }.navigationTitle("Dettaglio").navigationBarTitleDisplayMode(.inline).toolbar { ToolbarItem(placement: .cancellationAction) { Button("Chiudi") { dismiss() } }.self }.sheet(isPresented: $showingEdit) { EditAgendaItemView(item: item) } } }
