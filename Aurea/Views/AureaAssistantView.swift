...
struct AureaAssistantView: View {
    ...
    @State private var messages: [AssistantMessage] = [
        AssistantMessage(role: .assistant, text: "Ciao. Posso leggere i dati di Aurea, analizzarli e preparare semplici azioni da confermare.")
    ]
    ...
    var body: some View {
        NavigationStack {
            ...
            .confirmationDialog("Conferma azione", item: $pendingAction, titleVisibility: .visible) { action in
                Button("Conferma") { execute(action) }
    ...
    private func answer(to question: String) -> String {
        ...
        if containsAny(q, ["spesa piu alta", "spesa maggiore", "speso di piu", "movimento piu alto"]) {
            guard let highest = currentMonthTransactions.filter({ $0.type == .expense && $0.category != "Trasferimento" }).max(by: { valueInEUR($0) < valueInEUR($1) }) else {
        ...
        if containsAny(q, ["categoria", "ristor", "cibo", "trasport", "svago", "shopping"]) && containsAny(q, ["spes", "speso", "quanto"]) {
            if let requested = matchingCategory(in: q) {
                let value = expensesThisMonth(category: requested)
                return "Questo mese hai speso \(value.formatted(.currency(code: "EUR"))) in \(requested)."
            }
            let grouped = Dictionary(grouping: currentMonthTransactions.filter { $0.type == .expense && $0.category != "Trasferimento" }, by: { $0.category })
            guard let top = grouped.map({ ($0.key, $0.value.reduce(Decimal.zero) { $0 + valueInEUR($1) }) }).max(by: { $0.1 < $1.1 }) else {
                return "Non risultano spese questo mese."
            }
        ...
        if containsAny(q, ["mese scorso", "rispetto", "confront", "andando"]) && containsAny(q, ["spes", "mese", "andando"]) {
            ...
            let verb = difference > 0 ? "in più" : "in meno"
            return "Questo mese hai speso \(monthExpenses.formatted(.currency(code: "EUR"))), cioè \(absDecimal(difference).formatted(.currency(code: "EUR"))) \(verb) rispetto al mese scorso."
        }
        ...
        if containsAny(q, ["spes", "speso", "spendo", "uscit"]) && containsAny(q, ["mese", "mensil"]) {
        ...
        if containsAny(q, ["agenda", "impegn", "appuntament", "scadenz", "prossim", "cosa devo fare"]) {
            ...
            let preview = upcomingAgenda.prefix(4).map { item in
        ...
        if containsAny(q, ["patrimonio", "saldo totale", "soldi totali", "quanto possiedo"]) {
            return "Il tuo patrimonio attuale è \(FinancialEngine.netWorth(wallets: activeWallets).formatted(.currency(code: "EUR")))."
        }
    ...
    private func parseAction(_ input: String) -> PendingAssistantAction? {
        ...
        if containsAny(q, ["domani", "oggi"]) && (containsAny(q, ["alle", "ore", "appuntamento", "impegno", "ricordami", "aggiungi"]) || extractHour(from: q) != nil) {
            ...
            if q.contains("domani") { date = Calendar.current.date(byAdding: .day, value: 1, to: date) ?? date }
            let hour = extractHour(from: q)
            if let hour { date = Calendar.current.date(bySettingHour: hour.hour, minute: hour.minute, second: 0, of: date) ?? date }
        ...
        if let amount = extractAmount(from: q), !containsAny(q, ["quanto", "speso", "spese", "guadagnato", "patrimonio"]) {
            guard let wallet = activeWallets.first else { return nil }
            let title = cleanedTransactionTitle(input)
            guard !title.isEmpty else { return nil }
            let category = guessedCategory(for: q)
            return PendingAssistantAction(kind: .expense(title: title, amount: amount, category: category, wallet: wallet))
        }
    ...
    private func execute(_ action: PendingAssistantAction) {
        ...
        case let .expense(title, amount, category, wallet):
            let transaction = Transaction(type: .expense, amount: amount, category: category, title: title, wallet: wallet)
    ...
    private func expensesThisMonth(category: String) -> Decimal {
        currentMonthTransactions.filter { $0.type == .expense && normalized($0.category) == normalized(category) }.reduce(0) { $0 + valueInEUR($1) }
    }
    ...
    private func guessedCategory(for text: String) -> String {
        ...
        let rules: [(String, [String])] = [
            ("Cibo", ["pizza", "ristor", "pranzo", "cena", "bar", "caffe", "spesa supermercato"]),
            ("Trasporti", ["benzina", "treno", "bus", "taxi", "uber", "parcheggio"]),
            ("Svago", ["cinema", "concerto", "gioco", "aperitivo"]),
    ...
    private func extractHour(from text: String) -> (hour: Int, minute: Int)? {
        ...
        guard let regex = try? NSRegularExpression(pattern: pattern), let match = regex.firstMatch(in: text, range: NSRange(text.startIndex..., in: text)), let hourRange = Range(match.range(at: 1), in: text), let hour = Int(text[hourRange]), (0...23).contains(hour) else { return nil }
        var minute = 0
        if match.range(at: 2).location != NSNotFound, let minuteRange = Range(match.range(at: 2), in: text) { minute = Int(text[minuteRange]) ?? 0 }
    ...
    private func cleanedAgendaTitle(_ input: String) -> String {
        ...
        value = value.replacingOccurrences(of: "aggiungi", with: "").replacingOccurrences(of: "ricordami", with: "").replacingOccurrences(of: "domani", with: "").replacingOccurrences(of: "oggi", with: "")
...
private struct PendingAssistantAction: Identifiable {
    enum Kind {
        case expense(title: String, amount: Decimal, category: String, wallet: Wallet)
    ...
    var summary: String {
        switch kind {
        case let .expense(title, amount, category, wallet):
            return "Registra una spesa: \(title), \(amount.formatted(.currency(code: wallet.currencyCode))), categoria \(category), dal portafoglio \(wallet.name)."
        case let .agenda(title, date, hasTime):
