...
struct EditAgendaItemView: View {
    ...
    init(item: AgendaItem) {
        self.item = item; _title = State(initialValue: item.title); _note = State(initialValue: item.note); _type = State(initialValue: item.type); _date = State(initialValue: item.date); _hasTime = State(initialValue: item.hasTime); _repeatRule = State(initialValue: item.repeatRule); _reminder = State(initialValue: item.reminderMinutesBefore ?? 0); _priority = State(initialValue: item.priority)
    }
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Section("Dettagli") {
                    TextField("Titolo", text: $title); TextField("Note", text: $note, axis: .vertical); DatePicker("Data", selection: $date, displayedComponents: .date); Toggle("Orario", isOn: $hasTime)
    ...
    private func save() {
        item.title = title.trimmingCharacters(in: .whitespacesAndNewlines); item.note = note.trimmingCharacters(in: .whitespacesAndNewlines); item.type = type; item.date = date; item.hasTime = hasTime; item.repeatRule = repeatRule; item.reminderMinutesBefore = hasTime && reminder != 0 ? reminder : nil; item.priority = priority; AgendaNotificationManager.schedule(for: item); dismiss()
    }
}
