...
struct GoalDetailView: View {
    ...
    private var progress: Double {
        guard let target = goal.targetAmount, target > 0 else {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section("Obiettivo") {
                    ...
                    if let date = goal.targetDate {
                        LabeledContent("Scadenza", value: date.formatted(date: .abbreviated, time: .omitted))
                    }
                    ...
                    Section("Collegamento reale") {
                        if let wallet = goal.linkedWallet {
                            ...
                            Button {
                                ...
                                Label("Sincronizza dal saldo", systemImage: "arrow.triangle.2.circlepath")
                            }
                            Button(role: .destructive) {
                                goal.linkedWallet = nil
                            } label: {
                                Text("Scollega portafoglio")
                            }
                        } else {
                            Button {
                                showingWalletPicker = true
                            } label: {
                                Label("Collega un portafoglio", systemImage: "link")
                            }
                        }
                    }
                }
                ...
                if goal.targetAmount != nil && !goal.isCompleted {
                    Section {
                        Button { showingAddProgress = true } label: {
                            Label("Aggiungi risparmio manuale", systemImage: "plus.circle")
                        }
                    }
                }

                Section {
                    Button { goal.isCompleted.toggle() } label: {
                        Label(goal.isCompleted ? "Riapri obiettivo" : "Segna come completato", systemImage: goal.isCompleted ? "arrow.uturn.backward.circle" : "checkmark.circle")
                    }
                }
            ...
            .sheet(isPresented: $showingWalletPicker) {
                GoalWalletPicker(goal: goal, wallets: wallets.filter { !$0.isArchived })
            }
        }
    }
    ...
    private func syncFromWallet() {
        ...
        if let target = goal.targetAmount {
            goal.currentAmount = min(value, target)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Aggiungi") {
                        ...
                        if let target = goal.targetAmount, goal.currentAmount >= target {
