...
struct NetWorthCardView: View {
    ...
    private var currencyCode: String {
        wallets.first?.currencyCode ?? "EUR"
    }

    private var monthlyIncome: Decimal {
        FinancialEngine.totalIncome(from: monthlyTransactions)
    }

    private var monthlyExpenses: Decimal {
        FinancialEngine.totalExpenses(from: monthlyTransactions)
    }

    private var monthlyCashFlow: Decimal {
        FinancialEngine.cashFlow(from: monthlyTransactions)
    }
                    ...
                    HStack {
                        ...
                        if !monthlyTransactions.isEmpty {
                            HStack(spacing: 4) {
                                ...
                                Text(monthlyCashFlow, format: .currency(code: currencyCode))
                            }
